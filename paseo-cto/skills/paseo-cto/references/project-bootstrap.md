...
way an outcome does, with the CTO as the corrected party: the reviewer holds two returns, each one
leaving its line in the wave's `Plan review` section. A tree the reviewer still cannot accept after
the second is a scope question rather than a review question — accept with a recorded limitation and
